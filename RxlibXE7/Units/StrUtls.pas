...
begin
  ...
  while I <= SLen do begin
    while (I <= SLen) and
      {$IFDEF RX_UNICODE}
      CharInSet(Result[I], WordDelims)
      {$ELSE}
      (Result[I] in WordDelims)
      {$ENDIF}
    do Inc(I);
...
begin
  ...
  while (I < Length(S)) do begin
    ...
    then begin
      ...
      if
        {$IFDEF RX_UNICODE}
        CharInSet(Next, RomanChars)
        {$ELSE}
        (Next in RomanChars)
        {$ENDIF}
        and
        (RomanValues[Index] < RomanValues[Next])
      then begin
