...
uses Consts, {$IFDEF RX_D6} RTLConsts, {$ENDIF}
  {$IFNDEF WIN32} Str16, {$ENDIF} VclUtils, AniFile, RxConst,
  MaxMin, RxGConst;
...
var
  I: TGIFVersion;
  S: array[0..BufSize] of AnsiChar;
begin
  FVersion := gvUnknown;
  S[BufSize] := #0;
  Stream.Read(S, BufSize);
  if StrIComp(S, GIFSignature) <> 0 then GifError(LoadStr(SGIFVersion));
  Stream.Read(S, BufSize);
  StrLower(S);
  for I := Low(TGIFVersion) to High(TGIFVersion) do
    if CompareMem(@S, GIFVersionStr[I], BufSize) then begin
      FVersion := I;
      Break;
    end;
        ...
        begin
          ...
          if SeparatorChar = CHR_IMAGE_SEPARATOR then
            try
              ...
              if not
                (SeparatorChar in [CHR_EXT_INTRODUCER, CHR_IMAGE_SEPARATOR,
                   CHR_TRAILER])
              then begin
                SeparatorChar := #0;
...
var
  ...
  begin
    for I := 0 to Data.Count - 1 do
      WriteString(Stream, AnsiString(Data[I]));
    I := 0;
    Stream.Write(I, SizeOf(Byte));
  end;
...
begin
  ...
  try
    ...
    if FLooping and (FItems.Count > 1) then begin
      ...
      WriteString(Mem, AnsiChar(AE_LOOPING) + AnsiChar(LoByte(FRepeatCount)) +
        AnsiChar(HiByte(FRepeatCount)));
      Temp := 0;
      Mem.Write(Temp, SizeOf(Byte));
    end;
