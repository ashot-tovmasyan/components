...
{$ENDIF}
  ...
  private
    ...
    procedure SetAutoSizeRx(Value: Boolean);
  ...
  protected
    ...
    property AutoSize: Boolean read FAutoSize write SetAutoSizeRx default True;
...
uses SysUtils, Dialogs, {$IFDEF WIN32} CommCtrl, {$ELSE} Str16, {$ENDIF}
  VCLUtils, MaxMin, Consts, {$IFDEF RX_D6} RTLConsts, {$ENDIF}
  AppUtls {$IFDEF RX_D4}, ImgList, ActnList {$ENDIF};
...
procedure TRxCustomLabel.SetAutoSizeRx(Value: Boolean);
begin
...
var
  Sender: TControl;
begin
  if (Integer(Message.WParam) = FGroupIndex) and Parent.HandleAllocated then begin
