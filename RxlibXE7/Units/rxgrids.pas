...
{$IFDEF WIN32}
  ...
  {$ELSE} 	
  TInplaceEditStyle = (esSimple, esEllipsis, esPickList);
  {$ENDIF RX_D9}
...
{$IFDEF WIN32}
    ...
    function GetEditStyleRx(ACol, ARow: Longint): TInplaceEditStyle; dynamic;
...
uses SysUtils, VCLUtils, MaxMin, Consts, AppUtls;
...
begin
  with TRxDrawGrid(Grid) do begin
    ...
    EditStyle := GetEditStyleRx(Col, Row);
  end;
...
begin
  with Message do
    ...
    PtInRect(Rect(Width - FButtonWidth, 0, Width, Height), Point(XPos, YPos)) then Exit;
  inherited;
end;
...
function TRxDrawGrid.GetEditStyleRx(ACol, ARow: Longint): TInplaceEditStyle;
begin
  Result := esSimple;
