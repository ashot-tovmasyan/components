...
uses {$IFDEF WIN32} Bde, Windows, {$ELSE} DbiTypes, DbiProcs, {$ENDIF}
  Classes, SysUtils, DB, DBTables, StrUtls, BdeUtils, DBUtils;
...
uses Consts, DBConsts, {$IFDEF RX_D6} RTLConsts, {$ENDIF} Forms
  {$IFDEF RX_D3}, BDEConst {$ENDIF}
...
begin
  {$IFDEF RX_UNICODE}
  Result :=
    CharInSet(C, [' ', ',', ';', ')', #13, #10]) or CharInSet(C, Delims);
  {$ELSE}
...
function TRxQuery.GetRecord(Buffer: TRecordBuffer; GetMode: TGetMode;
