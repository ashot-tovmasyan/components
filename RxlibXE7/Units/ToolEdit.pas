...
uses ShellAPI, Consts, {$IFDEF RX_D3} ExtDlgs, {$ENDIF} RXCConst, VCLUtils,
  {$IFDEF RX_D6} Variants, RTLConsts, {$ENDIF}
  StrUtls, FileUtil, PickDate, MaxMin;
...
{$ENDIF RX_D4}
    ...
    try
      ...
      with ACanvas do begin
        ...
        S := AText;
        AWidth := TextWidth(S);
        TextMargins := EditorTextMargins(Editor);
        if PopupVisible then ALeft := TextMargins.X
        else begin
          ...
          case AAlignment of
            taLeftJustify:
              ALeft := TextMargins.X;
            taRightJustify:
              ALeft := ClientWidth - ButtonWidth - AWidth - TextMargins.X - 2;
            else
...
{$ENDIF}
        TextRect(R, ALeft, TextMargins.Y, S);
      end;
    finally
      ACanvas.Handle := 0;
