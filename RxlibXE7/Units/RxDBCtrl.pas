...
{$ENDIF} 
  ...
  Placemnt, DateUtil, DBCtrls, RxCtrls, CurrEdit, DbUtils;
...
uses SysUtils, StrUtls, Dialogs, ExtCtrls,
  DbConsts, {$IFDEF RX_D6} VDBConsts, {$ENDIF} AppUtls, VCLUtils,
  {$IFNDEF RX_D3} BdeUtils, {$ENDIF} PickDate, RxCalc, MaxMin,
  {$IFDEF RX_D6} Variants, {$ENDIF} RxDConst;
...
begin
  ...
  if
    {$IFDEF RX_UNICODE}
    CharInSet(Key, [#32..#255])
    {$ELSE}
    (Key in [#32..#255])
    {$ENDIF}
    and (FDataLink.Field <> nil) and not FDataLink.Field.IsValidChar(Key)
  then begin
    Beep;
    Key := #0;
  end;
...
begin
  ...
  if
    {$IFDEF RX_UNICODE}
    CharInSet(Key, [#32..#255]) and not CharInSet(Key, ['0'..'9'])
    {$ELSE}
    (Key in [#32..#255]) and not (Key in ['0'..'9'])
    {$ENDIF}
    and (FDataLink.Field <> nil)
    and (Key <> {$IFDEF USE_FORMATSETTINGS}FormatSettings.{$ENDIF}DateSeparator)
  then begin
    Beep;
    Key := #0;
  end;
...
begin
  if (csPaintCopy in ControlState) and (FDataLink.Field <> nil) then begin
    if FDataLink.Field.IsNull then begin
      S := GetDateFormat;
      S := ReplaceStr(ReplaceStr(ReplaceStr(ReplaceStr(S, '/',
