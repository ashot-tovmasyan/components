...
{ TRxCustomRichEdit }
  ...
  TRichStreamMode = (smSelection, smPlainRtf, smNoObjects{$IFNDEF RX_UNICODE}, smUnicode{$ENDIF});
...
type
  ...
  TFindTextEx = {$IFDEF RX_UNICODE}TFindTextExW{$ELSE}TFindTextExA{$ENDIF};
  ...
  TTextRange = {$IFDEF RX_UNICODE}TTextRangeW{$ELSE}TTextRangeA{$ENDIF};
...
{$ENDIF}
    procedure LoadFromFile(const FileName: string{$IFDEF RX_UNICODE}; Encoding: TEncoding{$ENDIF}); override;
    procedure LoadFromStream(Stream: TStream{$IFDEF RX_UNICODE}; Encoding: TEncoding{$ENDIF}); override;
    procedure SaveToFile(const FileName: string{$IFDEF RX_UNICODE}; Encoding: TEncoding{$ENDIF}); override;
    procedure SaveToStream(Stream: TStream{$IFDEF RX_UNICODE}; Encoding: TEncoding{$ENDIF});override;
...
{$IFDEF RX_UNICODE}
function StreamSave(dwCookie: {$IFDEF RX_D19}DWORD_PTR{$ELSE}Longint{$ENDIF};
...
{$IFDEF RX_UNICODE}
function StreamLoad(dwCookie: {$IFDEF RX_D19}DWORD_PTR{$ELSE}Longint{$ENDIF};
  pbBuff: PByte; cb: Longint; var pcb: Longint): Longint; stdcall;
var
    ...
    try
      if StreamInfo^.Converter <> nil then
        pcb := StreamInfo^.Converter.ConvertReadStream(StreamInfo^.Stream,
          {$IFDEF RX_UNICODE}TConversionBuffer{$ENDIF}(pBuff), cb);
      if pcb > 0 then
      begin
        pBuff[pcb] := #0;
...
procedure TRichEditStrings.LoadFromStream(Stream: TStream{$IFDEF RX_UNICODE}; Encoding: TEncoding{$ENDIF});
var
...
{$ENDIF}
    ...
    begin
      ...
      if (TextType and SF_RTF = SF_RTF) then
        TextType := SF_TEXT{$IFNDEF RX_UNICODE} or SF_UNICODE{$ENDIF}
      else
        TextType := SF_RTF;
{$IFDEF RX_UNICODE}
...
procedure TRichEditStrings.SaveToStream(Stream: TStream{$IFDEF RX_UNICODE}; Encoding: TEncoding{$ENDIF});
var
  EditStream: TEditStream;
  TextType: Longint;
  StreamInfo: TRichEditStreamInfo;
...
procedure TRichEditStrings.LoadFromFile(const FileName: string{$IFDEF RX_UNICODE}; Encoding: TEncoding{$ENDIF});
var
  Ext: string;
...
begin
{$IFNDEF VER90}
  Ext := {$IFDEF RX_UNICODE}WideLowerCase{$ELSE}AnsiLowerCaseFileName{$ENDIF}(ExtractFileExt(Filename));
{$ELSE}
...
{$ENDIF}
  ...
  try
    ...
    try
      ...
      inherited LoadFromFile(FileName{$IFDEF RX_UNICODE}, Encoding{$ENDIF});
    finally
      FFormat := SaveFormat;
    end;
  except
...
procedure TRichEditStrings.SaveToFile(const FileName: string{$IFDEF RX_UNICODE}; Encoding: TEncoding{$ENDIF});
var
  Ext: string;
  Convert: PRichConversionFormat;
  SaveFormat: TRichStreamFormat;
...
{$ENDIF}
  ...
  try
    ...
    try
      ...
      inherited SaveToFile(FileName{$IFDEF RX_UNICODE}, Encoding{$ENDIF});
    finally
      FFormat := SaveFormat;
    end;
  except
...
function TOEMConversion.ConvertReadStream(Stream: TStream;
  Buffer: {$IFDEF RX_UNICODE}TConversionBuffer{$ELSE}PChar{$ENDIF}; BufSize: Integer): Integer;
var
  Mem: TMemoryStream;
begin
  ...
  try
    ...
    Result := inherited ConvertReadStream(Stream,
      {$IFDEF RX_UNICODE}TConversionBuffer{$ELSE}PChar{$ENDIF}(Mem.Memory), BufSize);
    OemToCharBuff(PAnsiChar(Mem.Memory), {$IFDEF RX_UNICODE}PChar{$ENDIF}(Buffer), Result);
  finally
    Mem.Free;
  end;
end;
...
begin
  ...
  try
    Mem.SetSize(BufSize);
    CharToOemBuff({$IFDEF RX_UNICODE}PChar{$ENDIF}(Buffer), PAnsiChar(Mem.Memory), BufSize);
