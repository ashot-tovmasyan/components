...
uses {$IFDEF WIN32} Windows, {$ELSE} WinProcs, WinTypes, {$ENDIF} SysUtils,
  ...
  {$IFNDEF RX_D6} DsgnIntf, {$ELSE} DesignIntf, DesignEditors, {$ENDIF} RxDsgn;
...
function SelectDataSet(ADesigner:
 {$IFNDEF RX_D6}IFormDesigner{$ELSE}IDesignerHook{$ENDIF};
  const ACaption: string; ExcludeDataSet: TDataSet): TDataSet;
...
uses DbConsts, TypInfo, VclUtils, StrUtls, RxLConst,
...
begin
  ...
  DataSet := SelectDataSet(
    {$IFDEF RX_D6}IDesignerHook(Designer){$ELSE}Designer{$ENDIF},
    Caption, TDataSet(Component));
...
begin
  ...
  if (Field <> nil) then begin
    ...
    for I := Length(Temp) downto 1 do
      if not
        {$IFDEF RX_UNICODE}
        CharInSet(Temp[I], AlphaNumeric)
        {$ELSE}
        (Temp[I] in AlphaNumeric)
        {$ENDIF}
      then
