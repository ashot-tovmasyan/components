...
uses {$IFDEF WIN32} Windows, {$ELSE} WinTypes, {$ENDIF} Classes, SysUtils,
  {$IFNDEF RX_D6} DsgnIntf, {$ELSE} DesignIntf, DesignEditors, {$ENDIF}
...
{$ELSE}
procedure TPaintBoxEditor.EditProperty(const Prop: IProperty;
  var Continue: Boolean);
{$ENDIF}
begin
  if CompareText({$IFNDEF RX_D6}PropertyEditor{$ELSE}Prop{$ENDIF}.GetName,
                 'OnPaint') = 0 then begin
    {$IFNDEF RX_D6}PropertyEditor{$ELSE}Prop{$ENDIF}.Edit;
    Continue := False;
  end
  else
   inherited EditProperty(
    {$IFNDEF RX_D6} PropertyEditor, Continue, FreeEditor
            {$ELSE} Prop, Continue {$ENDIF}
    );
end;
...
begin
  try
    if FContinue and
      ( CompareText(
          {$IFNDEF RX_D6}PropertyEditor{$ELSE}Prop{$ENDIF}.GetName,
          'GLYPH') = 0
      ) then
    begin
      {$IFNDEF RX_D6}PropertyEditor{$ELSE}Prop{$ENDIF}.Edit;
...
var
  Components: {$IFNDEF RX_D6} TDesignerSelectionList;
                      {$ELSE} IDesignerSelections; {$ENDIF}
begin
  Components := {$IFNDEF RX_D6} TDesignerSelectionList
                        {$ELSE} TDesignerSelections {$ENDIF}.Create;
  try
    FContinue := True;
    Components.Add(Component);

    GetComponentProperties(Components, tkAny, Designer, CheckEdit
      {$IFDEF RX_D6} ,Nil {$ENDIF} );
  finally
    {$IFNDEF RX_D6}
    Components.Free;
    {$ENDIF}
