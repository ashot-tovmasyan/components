...
uses SysUtils, {$IFDEF WIN32} Windows, Bde, Registry, {$ELSE} WinTypes,
  WinProcs, DbiTypes, DbiProcs, DbiErrs, {$ENDIF} Classes, DB, DBTables,
  IniFiles, DBUtils;
...
uses Forms, Controls, Dialogs, Consts, DBConsts, RXDConst, VCLUtils,
  FileUtil, AppUtls, StrUtls, MaxMin, {$IFNDEF WIN32} Str16, {$ENDIF}
  {$IFDEF RX_D6} RTLConsts, {$ENDIF}
...
{$IFDEF RX_D3}
    if
      {$IFDEF RX_UNICODE}
      CharInSet(DatabaseName[I], LeadBytes)
      {$ELSE}
      (DatabaseName[I] in LeadBytes)
      {$ENDIF}
    then
      Inc(I)
    else
