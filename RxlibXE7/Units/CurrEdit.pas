...
uses Consts, StrUtls, VclUtils, MaxMin, RxCalc;
...
{$IFDEF USE_FORMATSETTINGS}with FormatSettings do {$ENDIF}
  for I := 1 to Length(Value) do
    if not
      {$IFDEF RX_UNICODE}
      CharInSet(Value[I], [DecimalSeparator, '-', '+', '0'..'9', 'e', 'E'])
      {$ELSE}
      (Value[I] in [DecimalSeparator, '-', '+', '0'..'9', 'e', 'E'])
      {$ENDIF}
    then
      Exit;
...
{$IFDEF USE_FORMATSETTINGS}with FormatSettings do begin{$ENDIF}
...
{$IFDEF USE_FORMATSETTINGS}end;{$ENDIF}
end;
...
begin
  ...
  S := TextToValText(S);
  DecPos := Pos(
    {$IFDEF USE_FORMATSETTINGS}FormatSettings.{$ENDIF}DecimalSeparator, S);
...
{$IFDEF USE_FORMATSETTINGS}with FormatSettings do begin{$ENDIF}
  if PopupVisible and
     {$IFDEF RX_UNICODE}
     CharInSet(UpCase(Key), ['0'..'9', DecimalSeparator, '.', ',',
        '+', '-', '*', '/', '_', '=', 'C', 'R', 'Q', '%', #8, #13])
     {$ELSE}
     (UpCase(Key) in ['0'..'9', DecimalSeparator, '.', ',',
        '+', '-', '*', '/', '_', '=', 'C', 'R', 'Q', '%', #8, #13])
     {$ENDIF}
  then begin
