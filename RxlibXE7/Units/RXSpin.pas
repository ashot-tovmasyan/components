...
{$IFDEF CBUILDER}
  TValueType = (vtInt, vtFloat, vtHex, vtUser);
{$ELSE}
  TValueType = (vtInteger, vtFloat, vtHex, vtUser);
{$ENDIF}
...
{$ENDIF}
  ...
  TRxSpinEditGetValueEvent = procedure(Sender: TObject;
    const Text: string; var Value: Extended) of object;
  TRxSpinEditSetValueEvent = procedure(Sender: TObject;
    var Text: string; const Value: Extended) of object;
  ...
  private
    ...
    FOnTopClick: TNotifyEvent;
    FOnBottomClick: TNotifyEvent;
    FOnGetValue: TRxSpinEditGetValueEvent;
    FOnSetValue: TRxSpinEditSetValueEvent;
{$IFDEF WIN32}
...
{$ENDIF}
    ...
    property OnBottomClick: TNotifyEvent read FOnBottomClick write FOnBottomClick;
    property OnTopClick: TNotifyEvent read FOnTopClick write FOnTopClick;
    property OnGetValue: TRxSpinEditGetValueEvent read FOnGetValue write FOnGetValue;
    property OnSetValue: TRxSpinEditSetValueEvent read FOnSetValue write FOnSetValue;
    property OnChange;
    property OnClick;
    property OnDblClick;
    property OnDragDrop;
...
uses {$IFDEF WIN32} CommCtrl, {$ENDIF} VCLUtils, StrUtls;
...
begin
  ...
  if ValueType = vtFloat then begin
    {$IFDEF USE_FORMATSETTINGS}with FormatSettings do{$ENDIF}
...
begin
 case ValueType of
  vtFloat:
    NewText := FloatToStrF(CheckValue(NewValue), ffFixed, 15, FDecimal);
  vtHex:
    NewText := IntToHex(Round(CheckValue(NewValue)), 1);
  {$IFDEF CBUILDER}
  vtInt:
  {$ELSE}
  vtInteger:
  {$ENDIF}
    NewText := IntToStr(Round(CheckValue(NewValue)));
  else
    NewText := '';
 end;
 if Assigned(FOnSetValue) then
   FOnSetValue(Self, NewText, NewValue);
 if ValueType = vtUser then FUserValue := NewValue;
 Text := NewText;
end;
