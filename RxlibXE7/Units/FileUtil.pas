...
uses {$IFDEF WIN32} Windows, {$ELSE} WinTypes, WinProcs, {$ENDIF} Messages,
  SysUtils, Classes, Consts, {$IFDEF RX_D6} RTLConsts, {$ENDIF} Controls;
...
{$IFDEF RX_D3}
    not
      {$IFDEF RX_UNICODE}
      CharInSet(AnsiLastChar(Result)^, [':', '\'])
      {$ELSE}
      (AnsiLastChar(Result)^ in [':', '\'])
      {$ENDIF}
    then
{$ELSE}
...
{$ENDIF}
  begin
    if (Length(Result) = 1) and
      {$IFDEF RX_UNICODE}
      CharInSet(UpCase(Result[1]), ['A'..'Z'])
      {$ELSE}
      (UpCase(Result[1]) in ['A'..'Z'])
      {$ENDIF}
    then
      Result := Result + ':\'
    else Result := Result + '\';
