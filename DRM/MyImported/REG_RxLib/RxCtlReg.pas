...
{$ENDIF DCS}
  {$IFDEF WIN32} RxRichEd, {$ENDIF} Menus, {вместо FiltEdit,} fltred {от Dream Controls}, StdCtrls, Buttons,
...
begin
  Components := {$IFDEF RX_D6}TDesignerSelections{$ELSE}TDesignerSelectionList{$ENDIF}.Create;
{$IFNDEF RX_D6}
  try
{$ENDIF}
    FContinue := True;
...
(*  это я тоже закомментировал, так как не знаю, зачем нужен TScroller
{$IFDEF CBUILDER}
...
//  RegisterNonActiveX([TScroller], axrComponentOnly); и это я закомментировал
{$ENDIF RX_D3}
...
//  RegisterPropertyEditor(TypeInfo(string), TFileNameEdit, 'Filter', TFilterProperty); //я закомментировал
//  вместо этого:
  RegisterPropertyEditor(TypeInfo(string), TFileNameEdit, 'Filter', TFilterPropertyEdit);
//
