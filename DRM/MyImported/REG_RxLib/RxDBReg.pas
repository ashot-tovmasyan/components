...
{ Property and component editors for data aware components }
  RegisterPropertyEditor(TypeInfo(string), TRxLookupControl, 'LookupField',
    TRxFieldProperty);
  //это тоже я добавил (почему-то не было)
  RegisterPropertyEditor(TypeInfo(string), TRxLookupControl, 'LookupDisplay',
    TRxFieldProperty);
  //
...
// посмотреть, что с RegisterComponentAction для редакторов полей
{$ENDIF DCS}
{$ENDIF RX_D3}
