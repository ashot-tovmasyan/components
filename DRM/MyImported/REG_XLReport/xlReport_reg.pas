...
uses MainCnct; (*ÏÐÈÂßÇÊÀ Ê DbProjectDirectory*)

{ Register }

procedure Register;
begin
  RegisterComponents('Print', [{TxlReportG2, }TxlReport]);
...
//      çäåñü ToolServices = nil
//      ïîýòîìó ÿ çàìåíèë íà
//      CurrentDir := ExtractFilePath(ParamStr(0));
      CurrentDir := DbProjectDirectory; (*ÏÐÈÂßÇÊÀ Ê DbProjectDirectory*)
//
...
//    if OpenDialog.Execute then SetStrValue(OpenDialog.FileName);
    if OpenDialog.Execute then SetStrValue(
      ExtractRelativePath(DbProjectDirectory, OpenDialog.FileName)); (*ÏÐÈÂßÇÊÀ Ê DbProjectDirectory*)
  finally
    OpenDialog.Free;
  end;
end;
