...
Var TeeChartEditorHook   : TTeeChartEditorHook=nil;
...
Uses {$IFNDEF LINUX}
     ...
     TeeEditCha, TeeEdiSeri, TeeEdiGene, TeeEdiGrad,

     {$IFDEF TEEDBREG}
     DBChart, DBEditCh, TeeDBEdit, TeeData,
     {$ENDIF}
     ...
     TeeGanttEdi, TeeShapeEdi, TeePrevi, TeExport
...
begin
  Case Index of
    0..3: TeeShowAboutBox{$IFNDEF D4}('',''){$ENDIF};
    4: Edit;
    5: ChartPreview(nil,Chart);
    6: TeeExport(Application,Chart);
   {$IFDEF TEESHOWCUSTOMAXES}
    7: ; //ShowCollectionEditor(Designer,Chart,Chart.CustomAxes,'CustomAxes');!!!!!!!!!!!!!111
   {$ENDIF}
   {$IFNDEF CLX}
     {$IFDEF D4}
      {$IFDEF BCB}
...
begin
  result:='';
  Case Index of
    0: result:=TeeMsg_Version{$IFDEF TEETRIAL}+' TRIAL'{$ENDIF};
...
begin
  EditChartBrush(nil,TChartBrush(GetOrdValue));
  Designer.Modified;
end;
...
begin
  EditTeeGradient(nil,TCustomTeeGradient(GetOrdValue));
end;
...
begin
  {$IFNDEF D4}
  EditTeeGradientCustom(nil,TBarSeriesGradient(GetOrdValue),True);
  {$ELSE}
  EditTeeGradient(nil,TBarSeriesGradient(GetOrdValue),True);
  {$ENDIF}
end;
...
begin
  ...
  RegisterComponents( TeeMsg_TeeChartPalette, [TSeriesDataSet] );
  {$ENDIF}
  ...
  RegisterComponents(TeeMsg_TeeChartPalette,[TButtonColor,TButtonPen]);
