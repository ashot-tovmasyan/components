{*****************************************}
{  TeeChart Pro                           }
{  Copyright (c) 1999-2001 David Berneda  }
...
uses {$IFNDEF LINUX}
     ...
     SysUtils, Classes, 
     {$IFDEF CLX}
     QGraphics, QControls, QForms, QStdCtrls, QExtCtrls, QMenus, QComCtrls, QImgList,
  ImgList, Controls, Menus, StdCtrls, TeeLisB, ComCtrls, ToolWin
     {$ELSE}
...
type
  TSeriesEditor = class(TDesignWindow)
...
type TTeeSeriesDesigner=class(TTeeCustomDesigner)
     private
...
begin
  ...
  begin
    ...
    try
      ...
      if Selected[t] then ComponentList.Add(ChartListBox1.Series[t]);
      if ComponentList.Count=0 then ComponentList.Add(ChartListBox1.Chart);
    except
      ComponentList.Free;
      raise;
    end;
...
begin
  case Key of
    ...
    #27:
      ...
      begin
        ComponentList:=TTeeDesignerList.Create;
        ComponentList.Add(ChartListBox1.Chart);
        SetSelection(ComponentList);
        Key := #0;
      end;
  end;
...
begin
  if Assigned(ChartListBox1.Chart) then FreeAndNil(ChartListBox1.Chart.Designer);
end;
