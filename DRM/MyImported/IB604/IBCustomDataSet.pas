...
procedure AddToLog(const atype, sql_text: string; Q: TIBCustomDataSet);
var
  B: string;
begin
  B :=  Format('%s  %s[%s]: %s', [FormatDateTime('hh:nn:ss', Time), atype, Q.Name, sql_text]);
  FLog.WriteBuffer(PChar(B)^, Length(B));
end;
*)
...
//    AddToLog('delete', FQDelete.SQL.Text, self); //ÓÚÎ‡‰Í‡!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    FQDelete.ExecQuery;
...
//    AddToLog('post', Qry.SQL.Text, self); //ÓÚÎ‡‰Í‡!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    Qry.ExecQuery;
...
begin
  ...
  try
    ...
    begin
//      AddToLog('ExecQuery', FQSelect.SQL.Text, self); //ÓÚÎ‡‰Í‡!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      FQSelect.ExecQuery;
    end;
  finally
    if DidActivate then
...
{initialization
  SLog := 'c:\Temp\Allegro'+ FormatDateTime('YYYY-MM-DD_HH-NN-SS',Now) + '.log';
  FLog := TFileStream.Create(SLog, fmCreate);
finalization
  Flog.Free;}
end.
