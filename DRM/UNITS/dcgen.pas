...
uses
  ...
  ExeImage, rxtypes,  //я добавил загрузку ресурсов Bitmap!
...
var
  BitmapResDLL: TExeImage;
  BitmapResList: TStringList;

procedure LoadBitmapResDLL;
var
  I,J: Integer;
  L: TResourceList;
begin
  for I := 0 to BitmapResDLL.Resources.Count - 1 do
  with BitmapResDLL.Resources[I] do
  if IsList and (List.Count > 0) and (List[0].ResType = rxtypes.rtBitmap) then
  begin
    L := BitmapResDLL.Resources[I].List;
    BitmapResList.Capacity := L.Count;
    for J := 0 to L.Count - 1 do
    if L[J].ResType = rxtypes.rtBitmap then
      BitmapResList.AddObject(L[J].Name, L[J]);
    break;
  end;
  BitmapResList.Sort;
end;
...
begin
{$IFDEF USE_ALLEGRORES_DLL}  (*----------поддержка W98-----я добавил-------*)
  DLLResIndex := BitmapResList.IndexOf(UpperCase(AClass.ClassName));
  if DLLResIndex = -1 then
    DLLResIndex := BitmapResList.IndexOf('_'+UpperCase(AClass.ClassName));
  if  DLLResIndex >= 0 then
  begin
    MemStr := TMemoryStream.Create;
    try
      TBitmapResource(BitmapResList.Objects[DLLResIndex]).SaveToStream(MemStr);
      MemStr.Seek(0,0);
...
initialization
{$IFDEF USE_ALLEGRORES_DLL} (*----------поддержка W98-----я добавил-------*)
  BitmapResDLL := TExeImage.CreateImage(nil, 'AllegroRes.dll');
  BitmapResList := TStringList.Create;
  LoadBitmapResDLL;
{$ENDIF}
